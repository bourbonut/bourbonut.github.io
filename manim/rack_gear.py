...
class Gear(Scene):
    def construct(self):
        ...
        profile = fast.revolution(
            gear.profile(m, z, alpha, ka, kf, False), angle_step, z
        )
...
class Rack(Scene):
    def construct(self):
        ...
        objs = VGroup(
            ...
            CustomDoubleArrow(
                -0.25 * step * X + ha * Y, -0.25 * step * X, tip_length=0.2
            ),
            CustomDoubleArrow(
                -0.25 * step * X, -0.25 * step * X - hf * Y, tip_length=0.2
            ),
            CustomDoubleArrow(
                -0.25 * step * X - hf * Y, -0.25 * step * X, tip_length=0.2
            ),
            ...
            CurvedDoubleArrow(A + (2 * ha + hf) * maths.u(PI / 2 - alpha), A + (2 * ha + hf) * Y, tip_length=0.2, radius=2 * ha + hf),
            MathTex("\\alpha").move_to(A + (2.5 * ha + hf) * maths.u(PI / 2 - 0.5 * alpha)),
        )
